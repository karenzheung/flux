...
class Objective extends Goal {
   ...
   */
  toJSON() {
    const inst = { 'shr.base.EntryType': { 'Value' : 'http://standardhealthrecord.org/spec/shr/careplan/Objective' } };
    ...
    if (this.occurrenceTimeOrPeriod != null) {
      inst['shr.core.OccurrenceTimeOrPeriod'] = typeof this.occurrenceTimeOrPeriod.toJSON === 'function' ? this.occurrenceTimeOrPeriod.toJSON() : this.occurrenceTimeOrPeriod;
    }
    if (this.status != null) {
    ...
    if (this.plannedActivities != null) {
      inst['shr.careplan.PlannedActivities'] = this.plannedActivities.map(f => f.toJSON());
    }
    if (this.actualActivities != null) {
      inst['shr.careplan.ActualActivities'] = this.actualActivities.map(f => f.toJSON());
    }
    if (this.goal != null) {
      inst['shr.careplan.Goal'] = this.goal.map(f => f.toJSON());
    }
    if (this.resultTargeted != null) {
      inst['shr.careplan.ResultTargeted'] = typeof this.resultTargeted.toJSON === 'function' ? this.resultTargeted.toJSON() : this.resultTargeted;
    }
    if (this.resultAchieved != null) {
      inst['shr.careplan.ResultAchieved'] = this.resultAchieved.map(f => f.toJSON());
    }
    if (this.relatedFinding != null) {
      inst['shr.finding.RelatedFinding'] = this.relatedFinding.map(f => f.toJSON());
    }
    return inst;
  }
}
