...
class MedicationChange extends Action {
  ...
  toJSON() {
    ...
    inst['shr.base.EntryType'] = { 'Value' : 'http://standardhealthrecord.org/spec/shr/medication/MedicationChange' };
    ...
    if (this.medicationBeforeChange != null) {
      inst['shr.medication.MedicationBeforeChange'] = this.medicationBeforeChange.map(f => f.toJSON());
    }
    if (this.medicationAfterChange != null) {
      inst['shr.medication.MedicationAfterChange'] = this.medicationAfterChange.map(f => f.toJSON());
    }
    return inst;
  }
}
